 ...
 * Description:
 */
class RatioRelativeLayout constructor(context: Context, attrs: AttributeSet? = null) : RelativeLayout(context, attrs), RatioInterface {
    ...
    init {
        if (attrs != null) {
            val array = context.obtainStyledAttributes(attrs, R.styleable.RatioRelativeLayout)
            mAxis = findAxis(array.getInt(R.styleable.RatioRelativeLayout_axis, 0))
            mVerticalRatio = array.getFloat(R.styleable.RatioRelativeLayout_verticalRatio, 1f)
            mHorizontalRatio = array.getFloat(R.styleable.RatioRelativeLayout_horizontalRatio, 1f)
            array.recycle()
        }
    }
