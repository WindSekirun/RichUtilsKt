 ...
 * Description:
 */
class RatioView constructor(context: Context, attrs: AttributeSet? = null) : View(context, attrs), RatioInterface {
    ...
    init {
        if (attrs != null) {
            val array = context.obtainStyledAttributes(attrs, R.styleable.RatioView)
            mAxis = findAxis(array.getInt(R.styleable.RatioView_axis, 0))
            mVerticalRatio = array.getFloat(R.styleable.RatioView_verticalRatio, 1f)
            mHorizontalRatio = array.getFloat(R.styleable.RatioView_horizontalRatio, 1f)
            array.recycle()
        }
    }
