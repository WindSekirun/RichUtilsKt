 ...
 * Description:
 */
class RatioLinearLayout constructor(context: Context, attrs: AttributeSet? = null) : LinearLayout(context, attrs), RatioInterface {
    ...
    init {
        if (attrs != null) {
            val array = context.obtainStyledAttributes(attrs, R.styleable.RatioLinearLayout)
            mAxis = findAxis(array.getInt(R.styleable.RatioLinearLayout_axis, 0))
            mVerticalRatio = array.getFloat(R.styleable.RatioLinearLayout_verticalRatio, 1f)
            mHorizontalRatio = array.getFloat(R.styleable.RatioLinearLayout_horizontalRatio, 1f)
            array.recycle()
        }
    }
